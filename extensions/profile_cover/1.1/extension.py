...
def is_user_profile(fragment):
    ...
    if dialog_id > 0:
        return True
    try:
        user_id = int(field(fragment, "userId", 0) or 0)
        chat_id = int(field(fragment, "chatId", 0) or 0)
...
def relative_top(fragment):
    ...
    if avatar is not None:
        try:
            value = field(avatar, "relativeTop", None)
            if value is not None:
                return float(value)
        except Exception:
            pass
    try:
        return float(field(fragment, "currentExpandAnimatorValue", 1.0))
    except Exception:
        return 1.0
...
def force_expanded(fragment):
    ...
    if list_view is None:
        return
    try:
        layout = field(fragment, "layoutManager")
        width = int(list_view.getMeasuredWidth())
        if layout is not None and width > 0:
            status = int(getattr(AndroidUtilities, "statusBarHeight", dp(24)))
            offset = max(dp(88), width - dp(56) - status)
            layout.scrollToPositionWithOffset(0, offset - int(list_view.getPaddingTop()))
        else:
...
def make_overlay(fragment, root):
    ...
    try:
        ...
        try:
            own_id = int(field(fragment, "userId", 0) or 0)
            edit.setVisibility(View.VISIBLE if own_id > 0 else View.GONE)
        except Exception:
            pass
...
class CreateHook(MethodHook):
    def after_hooked_method(self, param):
        ...
        if _API is None:
            return
        _API.schedule(lambda: force_expanded(fragment), 80)
        _API.schedule(lambda: force_expanded(fragment), 320)
        _API.schedule(lambda: decorate(fragment, root), 120)
        _API.schedule(lambda: decorate(fragment, root), 420)
...
def on_load(api):
    ...
    if cls is None:
        raise RuntimeError("Profile screen class not found")

    api.hook_all_methods(cls, "createView", CreateHook())

    scroll = ScrollHook()
    for name in ("updateScrollLayout", "checkListViewScroll", "setAvatarExpandProgress"):
        try:
            api.hook_all_methods(cls, name, scroll)
        except Exception:
            pass
    ...
    api.log("Profile Cover 1.1 loaded on " + str(class_name))
...
def on_unload(api):
    ...
    cleanup()
    if api is not None:
        api.log("Profile Cover 1.1 unloaded")
    _API = None
